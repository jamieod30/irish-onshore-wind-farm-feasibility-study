...
SITE_COMPARISON_FILE = ANALYSIS_DIR / "site_comparison.csv"

SCORING_OUTPUT_FILE = METADATA_DIR / "site_selection_scoring.csv"
RANKING_OUTPUT_FILE = METADATA_DIR / "site_selection_ranking.csv"

METADATA_DIR.mkdir(parents=True, exist_ok=True)

# Engineering-weighted scoring
# Resource strength is deliberately dominant because it is the main
# driver of wind farm energy yield and project value at screening stage.
WEIGHTS = {
    "resource_score": 0.45,
...
def min_max_score(series: pd.Series, higher_is_better: bool = True) -> pd.Series:
    """
    Scale a metric to a 1-5 score using min-max normalisation.
...
def build_resource_index(df: pd.DataFrame) -> pd.Series:
    """
    Create a composite resource index from:
    - mean_ws_100
    - mean_power_density
    - weibull_c

    Each component is normalised 0-1 and averaged equally.

    This avoids over-relying on a single wind-strength variable while
    keeping the ranking clearly resource-led.
    """
...
def shear_band_score(alpha: float) -> float:
    """
    Banded engineering screening score for annual mean wind shear exponent.

    Screening logic:
    - very low shear is not especially beneficial for hub-height gain
    - moderate shear is preferred because it can support stronger
      hub-height winds without immediately indicating elevated concern
    - high shear is penalised because it may indicate increased
      loading asymmetry, roughness effects, or operational caution

    This is a screening heuristic for public-data site comparison.
    It is not a substitute for turbine-specific inflow assessment,
    IEC classification, or OEM load evaluation.
    """
    if pd.isna(alpha):
        return np.nan
...
def main() -> None:
    ...
    # -----------------------------------------------------------------
    # Derived site-selection metrics
    # -----------------------------------------------------------------
